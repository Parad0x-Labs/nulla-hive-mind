...
- configured provider bootstrap is now shared and real on both sides: `KIMI_API_KEY` auto-registers a remote Kimi queen lane, `VLLM_BASE_URL` auto-registers a local `vllm-local` queen lane, and runtime backbone snapshots plus API startup now consume the same seam instead of drifting between “routing says it exists” and “bootstrap never registered it”
