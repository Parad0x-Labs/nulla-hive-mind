...
def dispatch_operator_action(
    ...
    *,
    ...
    return OperatorActionResult(
        ok=False,
        status="unsupported",
        response_text="I recognized an operator action request, but that action is not wired yet.",
...
def _handle_list_tools(
    ...
    *,
    ...
    unavailable = [tool for tool in tools if not tool.get("available")]
    lines = ["Available tool inventory:"]
    for tool in available:
        flag = "approval required" if tool.get("destructive") else "read-only"
        lines.append(f"- {tool['tool_id']} ({tool['category']}, {flag}): {tool['description']}")
    if unavailable:
        lines.append("")
        lines.append("Configured but currently unavailable:")
        for tool in unavailable:
            lines.append(f"- {tool['tool_id']}: missing runtime credentials or platform support")
    audit_logger.log(
        "operator_action_list_tools",
        target_id=task_id,
        target_type="task",
        details={"available_tool_ids": [tool["tool_id"] for tool in available]},
    )
    return OperatorActionResult(
        ok=True,
        status="reported",
