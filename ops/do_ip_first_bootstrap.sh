...
run_ssh "$WATCH_IP" "python3 -c \"import json; tok='${CLUSTER_MEET_TOKEN}'; payload={'node_id':'watch-edge-1','public_url':'https://nullabook.com','bind_host':'127.0.0.1','bind_port':8788,'request_timeout_seconds':6,'upstream_base_urls':['https://${EU_IP}:8766','https://${US_IP}:8766','https://${APAC_IP}:8766'],'auth_tokens_by_base_url':{'https://${EU_IP}:8766':tok,'https://${US_IP}:8766':tok,'https://${APAC_IP}:8766':tok},'tls_insecure_skip_verify':True}; json.dump(payload,open('/tmp/watch-http.json','w'),indent=2); print('Watch http config updated')\""
