...
- DHT lookup candidates now also prefer fresh peers over stale ones instead of treating age-blind XOR closeness as good enough, referral-only `NODE_FOUND` / `BLOCK_FOUND` endpoints are now candidate-only instead of authoritative transport rows, and weaker referral gossip no longer refreshes observed-peer liveness or stale-bucket freshness as if it were a real live contact.
