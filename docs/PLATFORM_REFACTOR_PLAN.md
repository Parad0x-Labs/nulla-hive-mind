...
- keep validation followup truth real too: explicit pytest/ruff debug requests should route through `workspace.run_tests` / `workspace.run_lint`, and failed validation results must feed source inspection or diagnostic search instead of dead-ending after the first red run
